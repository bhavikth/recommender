...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
	...
	public TFIDFModel get() {
		...
		for (long item : items) {
			...
			// TODO Populate the work vector with the number of times each tag
			// is applied to this item.
			List<String> tagsPerItem = dao.getItemTags(item);
			for (String tag : tagsPerItem) {
			...
			// TODO Increment the document frequency vector once for each unique
			// tag on the item.
			Set<String> uniqueTagsPerItem = new HashSet<String>(tagsPerItem);
			for (String tag : uniqueTagsPerItem) {
				long tagId = tagIds.get(tag);
		...
		for (Map.Entry<Long, MutableSparseVector> entry : itemVectors
			...
			for (VectorEntry tagInItem : tv.fast()) {
				double tf = tagInItem.getValue();
				double idf = docFreq.get(tagInItem.getKey());
				tv.set(tagInItem, tf * idf);
			}
			// TODO Normalize the TF-IDF vector to be a unit vector
			// HINT The method tv.norm() will give you the Euclidian length of
			// the vector
			double itemVecSize = tv.norm();
			for (VectorEntry tagInItem : tv.fast()) {
				tv.set(tagInItem, tagInItem.getValue() / itemVecSize);
			}
			assert (tv.norm() == 1);
