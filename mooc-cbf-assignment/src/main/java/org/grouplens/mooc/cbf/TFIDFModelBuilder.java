...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        Map<String, Long> tagIds = buidlTagIdMap();
        ...
        int documentCount = itemVectors.size();
        for (VectorEntry e: docFreq.fast()) {
            // TODO Update this document frequency entry to be a log-IDF value
        	int termDocCount = (int) e.getValue();
        	double idf = Math.log(documentCount/(termDocCount+1));
        ...
        for (Map.Entry<Long,MutableSparseVector> entry: itemVectors.entrySet()) {
            ...
            for (VectorEntry tve:tv.fast()){
            	double termFreq = tve.getValue();
            	double idf = docFreq.get(tve.getKey());
            	tv.set(tve.getKey(), termFreq*idf);
            }
            ...
            for (VectorEntry tve:tv.fast()){
            	tv.set(tve.getKey(), tve.getValue()/tv.norm());
            }
     ...
     */
    private Map<String,Long> buidlTagIdMap() {
