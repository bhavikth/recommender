...
public class TFIDFItemScorer extends AbstractItemScorer {
	...
	public void score(long user, @Nonnull MutableSparseVector output) {
		...
		for (VectorEntry e : output.fast(VectorEntry.State.EITHER)) {
			...
			// TODO Compute the cosine of this item and the user's profile,
			// store it in the output vector
			double cos = userVector.dot(iv) / userVector.norm() * iv.norm();
			output.set(e.getKey(), cos);
	...
	private SparseVector makeUserVector(long user) {
		...
		for (Rating r : userRatings) {
			if (r.getPreference() != null) {
				userAvg += r.getPreference().getValue();
				ratingsSize++;
			}
		}
		userAvg = userAvg / ratingsSize;

		// Iterate over the user's ratings to build their profile
		for (Rating r : userRatings) {
			// In LensKit, ratings are expressions of preference
			Preference p = r.getPreference();
			// We'll never have a null preference. But in LensKit, ratings can
			// have null
			...
			if (!CBFMain.weighted) {
				if (p != null && p.getValue() >= 3.5) {
					...
					// profile
					SparseVector itemVector = model
							.getItemVector(p.getItemId());
					profile.add(itemVector);

				}
			} else {
				if (p != null) {
					double weight = p.getValue() - userAvg;
					SparseVector itemVector = model
							.getItemVector(p.getItemId());
					for (VectorEntry ive : itemVector.fast()) {
						profile.add(ive.getKey(), weight * ive.getValue());
					}
				}
			}
		}
